...
class Logger(object):
    def __init__(self):
        ...
        self.log = open("/Users/Jeff/Desktop/model_21_svm_ploy_3_result.txt", "w")
...
for i in range(10): #do it 10 times to get loss stats
    ...
    for c in c_list:
        ...
        gamma_list = [ pow(10, -3), pow(10, -2), pow(10, -1),
                       pow(10, 0),
                       pow(10, 1), pow(10, 2), pow(10, 3)]
        ...
        for gamma in gamma_list:
            ...
            #learning cmd
            learn_cmd_poly_3 = "./svm-train -c %s -t 1 -d 3 -g %s -e 0.1 %s %s" % (c, gamma, train_dir, model_dir)

            #classifying set a command
            classify_a_cmd_poly_3 = "./svm-predict %s %s %s" % (test_a_dir, model_dir, tag_a_dir)

            #classifying set b command
            classify_b_cmd_poly_3 = "./svm-predict %s %s %s" % (test_b_dir, model_dir, tag_b_dir)

            #learning
            print ""
            print "------------------------------------------model %s, round %s, c=%s, gamma=%s, learning...-----------------------------------------------------" % (model, round, c, gamma)
            print datetime.datetime.now()
            learn_result = subprocess_cmd(learn_cmd_poly_3)
            print ""

            #classyfying set a
            print ""
            print "------------------------------------------model %s, round %s, c=%s, gamma=%s, classifying_a...------------------------------------------------" % (model, round, c, gamma)
            print datetime.datetime.now()
            print ""
            classify_a_result = subprocess_cmd(classify_a_cmd_poly_3)
            print classify_a_result

            #classfying set b
            print ""
            print "------------------------------------------model %s, round %s, c=%s, gamma=%s, classifying_b...------------------------------------------------" % (model, round, c, gamma)
            print datetime.datetime.now()
            print ""
            classify_b_result = subprocess_cmd(classify_b_cmd_poly_3)
